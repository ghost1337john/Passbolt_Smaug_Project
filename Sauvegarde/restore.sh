...
sudo cp -a "$WORKDIR/database_volume/." /app/passbolt/database_volume/
sudo cp -a "$WORKDIR/gpg_volume/." /app/passbolt/gpg_volume/
sudo cp -a "$WORKDIR/jwt_volume/." /app/passbolt/jwt_volume/
sudo cp -a "$WORKDIR/certs/." /app/passbolt/certs/

echo "[5/6] Verification des permissions..."
sudo chown -R 999:999 /app/passbolt/database_volume
sudo chown -R 999:999 /app/passbolt/gpg_volume
sudo chown -R 999:999 /app/passbolt/jwt_volume
sudo chown -R 999:999 /app/passbolt/certs
